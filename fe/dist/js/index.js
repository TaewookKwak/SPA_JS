...
function createDom(node) {
  var dom = node.type == "TEXT_ELEMENT" ? document.createTextNode("") : document.createElement(node.type);
...
function commitWork(node) {
  ...
  var domParentNode = node.parent; // 부모 node를 찾음

  while (!domParentNode.dom) {
    // 부모 node의 DOM이 없으면 부모 node를 찾음
    domParentNode = domParentNode.parent;
  }
  var domParent = domParentNode.dom; // 부모 node의 DOM을 찾음
...
var nextTask = null; // 다음 작업 단위
var rootNode = null; // 현재 루트 요소
var workInProgressRoot = null; // 작업 중인 루트 요소
var deletions = null; // 삭제할 요소

// workLoop : 브라우저의 여유 시간에 맞춰 작업을 수행합니다. 할 일이 남아있으면 performUnitOfWork를 호출하여 다음 작업을 진행합니다.
function workLoop(deadline) {
  var shouldYield = false; // 현재 작업을 계속할지, 중단할지를 결정하는 플래그입니다.
...
function performUnitOfWork(node) {
  var isFunctionComponent = node.type instanceof Function;
...
function updateFunctionComponent(node) {
  ...
  var children = [node.type(node.props)]; // 컴포넌트의 타입(함수)을 호출하여 자식 요소를 생성합니다. node.type은 컴포넌트 함수이고, node.props는 해당 컴포넌트에 전달된 속성입니다.
...
function useState(initial) {
  var oldHook = currentNode.alternate && currentNode.alternate.hooks && currentNode.alternate.hooks[currentHookIndex];
  var hook = {
...
function reconcileChildren(currentNode, elements) {
  var index = 0; // index는 현재 처리 중인 요소의 인덱스를 추적합니다
  var oldNode = currentNode.alternate && currentNode.alternate.child; // oldNode는 이전 렌더링에서의 자식 node를 가져옵니다. currentNode.alternate가 존재하는 경우에만 이전 node를 참조합니다.
  var prevSibling = null; //prevSibling은 새로 생성되는 node의 형제를 추적합니다
  ...
  while (index < elements.length || oldNode != null) {
    var _element = elements[index]; // 현재 인덱스에 해당하는 새로운 요소를 가져옵니다
    var newNode = null; // newNode는 새로 생성될 node를 저장할 변수입니다

    var sameType = oldNode && _element && _element.type == oldNode.type; // 현재 요소와 이전 node가 모두 존재하는 경우, 두 요소의 유형이 같은지 확인합니다.
